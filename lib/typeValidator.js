...
TypeValidator.prototype.setType = function setType (type)
{
    ...
    if (typeof type !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип типа параметра (ожидается: "string", принято: "' + typeof type + '").');
    if (!(type in typesMap))
        throw new $swiftErrors.ValueError('Недопустимое значение типа параметра (ожидается: "' + Object.keys(typesMap).join('"|"') + '", принято: "' + type + '").');
    //
    // задание типа параметра
    //
    this._type = type;
...
TypeValidator.prototype.validate = function validate (data)
{
    ...
    // проверка параметров
    //
    if (this._type === null) throw new $swiftErrors.SystemError('Не задан тип параметра.');
    //
    // валидация параметра
    //
