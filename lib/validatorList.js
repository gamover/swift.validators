...
ValidatorList.prototype.setValidatorList = function setValidatorList (validatorList)
{
    ...
    if (!$swiftUtils.type.isArray(validatorList))
        throw new $swiftErrors.TypeError('Список валидаторов должен иметь тип "array" ' +
            '(принято: "' + typeof validatorList + '").');
    for (var i = 0, n = validatorList.length; i < n; i++)
        if (!(validatorList[i] instanceof SwiftValidator))
            throw new $swiftErrors.TypeError('Валидатор validatorList[' + i + '] должен иметь тип "SwiftValidator" ' +
                '(принято: "' + typeof validatorList[i] + '").');
...
ValidatorList.prototype.addValidator = function addValidator (validator)
{
    ...
    if (!(validator instanceof SwiftValidator))
        throw new $swiftErrors.TypeError('Валидатор должен иметь тип "SwiftValidator" ' +
            '(принято: "' + typeof validator + '").');
    //
